...
pub async fn search_stocks(
    ...
    // `q` is `Option` at the DTO level so the missing-parameter case yields
    // our canonical `application/problem+json` 400 instead of a framework 400.
    let raw = query
        .into_inner()
        .q
        .ok_or_else(|| AppError::BadRequest("query parameter 'q' is required".to_string()))?;
    // Garde already enforced length(2..=50) on the raw value; trimming may
    // leave fewer than 2 visible characters, in which case the search would
    // be effectively unselective and we reject it.
    let q = raw.trim();
    if q.len() < 2 {
    ...
    let (stocks, truncated) = state.stock_service.search(q.to_string()).await?;
