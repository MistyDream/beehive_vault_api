...
pub async fn find_by_id(db: &Db, catalog_id: i32) -> Result<MetricCatalog, DbError> {
    db.exec(move |conn| {
...
pub async fn find_by_key(db: &Db, key: String) -> Result<MetricCatalog, DbError> {
    db.exec(move |conn| {
        let row = metrics_catalog::table
...
pub async fn find_by_category(
    db: &Db,
    category: String,
) -> Result<Vec<MetricCatalog>, DbError> {
    db.exec(move |conn| {
...
pub async fn list_all(db: &Db) -> Result<Vec<MetricCatalog>, DbError> {
    db.exec(move |conn| {
        let rows = metrics_catalog::table
            .select(MetricCatalogRow::as_select())
            .order(metrics_catalog::key.asc())
...
pub async fn delete(db: &Db, catalog_id: i32) -> Result<bool, DbError> {
    db.exec(move |conn| {
        let count =
