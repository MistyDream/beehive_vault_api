...
pub async fn find_by_id(db: &Db, benchmark_id: i32) -> Result<SectorBenchmark, DbError> {
    db.exec(move |conn| {
...
/// Returns `DbError::Diesel(NotFound)` if no benchmark exists.
pub async fn find_latest_by_sector_and_metric(
    db: &Db,
    sector: String,
    metric_key: String,
) -> Result<SectorBenchmark, DbError> {
    db.exec(move |conn| {
        let row = sector_benchmarks::table
            .filter(
...
pub async fn find_by_sector(
    db: &Db,
    sector: String,
) -> Result<Vec<SectorBenchmark>, DbError> {
    db.exec(move |conn| {
    ...
    db.exec(move |conn| {
        let row = diesel::insert_into(sector_benchmarks::table)
            ...
            .set((
                ...
                sector_benchmarks::fetched_at.eq(excluded(sector_benchmarks::fetched_at)),
            ))
            .returning(SectorBenchmarkRow::as_returning())
            .get_result(conn)?;
        Ok(SectorBenchmark::from(row))
...
pub async fn delete(db: &Db, benchmark_id: i32) -> Result<bool, DbError> {
    db.exec(move |conn| {
        let count =
...
pub async fn delete_by_sector(db: &Db, sector: String) -> Result<usize, DbError> {
    db.exec(move |conn| {
        let count = diesel::delete(
            sector_benchmarks::table.filter(sector_benchmarks::sector.eq(&sector)),
        )
        .execute(conn)?;
        Ok(count)
    })
